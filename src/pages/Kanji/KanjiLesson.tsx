...
export const KanjiLesson = () => {
  ...
  if (selectedKanjiIds.length > 0) {
    ...
    // remove duplicates by kanji + hiragana
    const seen = new Set();
    displayedVocab = tempVocab.filter(v => {
      const key = v.kanji + v.hiragana;
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex flex-wrap gap-2">
                <button 
                  ...
                  className={`flex items-center gap-2 text-white px-4 py-2 rounded-xl text-xs font-bold transition-colors shadow-lg ${isSelectMode ? 'bg-slate-500 hover:bg-slate-600 shadow-slate-500/30' : theme.btnBg}`}
                >
                  {isSelectMode ? <X size={14} /> : <Check size={14} />} 
          ...
          <div className="w-full lg:w-[40%] flex flex-col gap-4">
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-sm font-black text-slate-800 dark:text-slate-100 uppercase tracking-widest">
                VOCABULARY ({displayedVocab.length})
              </h2>
