...
import { 
  ArrowRight, Lock, Mail, UserRound, Eye, EyeOff, 
  Sparkles, AlertCircle, Flower2, Gift
} from 'lucide-react';
...
export const Register = () => {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const errorMsg = nextError instanceof Error ? nextError.message : 'Không thể đăng ký.';
      setError(errorMsg);
  ...
  const handleGoogle = useCallback(async (credential: string) => {
    ...
    try {
      ...
      const errorMsg = nextError instanceof Error ? nextError.message : 'Không thể đăng ký bằng Google.';
      setError(errorMsg);
      setMascotState('error');
    } finally {
      setIsSubmitting(false);
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#FFF8F3] text-slate-900 flex items-center justify-center p-4 sm:p-6 lg:p-8 font-sans">
      ...
      {/* ========================================================================= */}
      {/* 1. BACKGROUND NHẬT BẢN THƠ MỘNG: GPU CONTAINED */}
      {/* ========================================================================= */}
      ...
      {/* ========================================================================= */}
      {/* 2. MAIN COMPOSITION: SÂN KHẤU LINH VẬT (TRÁI) & CARD ĐĂNG KÝ (PHẢI) */}
      {/* ========================================================================= */}
      <div className="relative z-10 w-full max-w-[1080px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-center">
        ...
        >
          ...
          <p className="text-sm font-semibold text-slate-600 max-w-sm mb-4 leading-relaxed">
            Tạo tài khoản miễn phí để lưu toàn bộ tiến độ, luyện phát âm AI và chinh phục N5/N4 dễ dàng.
          </p>
          ...
          <div className="hidden lg:flex items-center gap-3 mt-3 text-xs font-bold text-slate-500">
            <span className="flex items-center gap-1.5 bg-white/80 px-3.5 py-1.5 rounded-full border border-orange-100/90 shadow-xs">
              <Gift size={14} className="text-orange-500" /> Học miễn phí N5
            </span>
            <span className="flex items-center gap-1.5 bg-white/80 px-3.5 py-1.5 rounded-full border border-orange-100/90 shadow-xs">
              <Flower2 size={14} className="text-pink-500" /> Lộ trình bài bản
            </span>
          </div>
        </motion.section>
        ...
        {/* ----------------------------------------------------------------------- */}
        {/* CỘT PHẢI: REGISTER CARD GLASSMORPHISM CHUẨN HIỆN ĐẠI (7 COLS) */}
        {/* ----------------------------------------------------------------------- */}
        <motion.section 
          ...
          <div className="relative overflow-hidden rounded-[34px] border border-white/90 bg-white/82 p-6 sm:p-9 shadow-[0_24px_70px_rgba(251,146,60,0.14)] backdrop-blur-2xl ring-1 ring-inset ring-white/90">
            ...
            <div className="relative mb-6">
              ...
              <p className="text-xs sm:text-sm font-semibold text-slate-500 mt-1">
                Rất vui được gặp bạn! Cùng khởi tạo hành trình học tiếng Nhật nhé.
              </p>
            </div>
            ...
            <form onSubmit={submit} className="relative space-y-3.5">
              ...
              <div>
                ...
                <div className="relative flex items-center rounded-2xl border border-slate-200/90 bg-white/95 px-4 py-3.5 shadow-xs transition-colors duration-150 focus-within:border-orange-400 focus-within:ring-4 focus-within:ring-orange-100/70">
                  ...
                  <input
                    ...
                    placeholder="Tên của bạn (vd: Minh Hoàng)"
                    required
                    autoComplete="name"
                  />
                </div>
              ...
              <div>
                ...
                <div className="relative flex items-center rounded-2xl border border-slate-200/90 bg-white/95 px-4 py-3.5 shadow-xs transition-colors duration-150 focus-within:border-amber-400 focus-within:ring-4 focus-within:ring-amber-100/70">
                  <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-amber-50 text-amber-500 mr-3">
                    <Lock size={18} />
                  </div>
                  <input
              ...
              {/* NÚT ĐĂNG KÝ CHÍNH (TACTILE SPRING BUTTON) */}
              <motion.button
                type="submit"
                ...
                className="group relative overflow-hidden flex w-full items-center justify-center gap-2.5 rounded-2xl bg-gradient-to-r from-orange-500 via-rose-500 to-pink-500 px-6 py-4 text-base font-black text-white shadow-[0_12px_28px_rgba(249,115,22,0.35)] hover:shadow-[0_16px_36px_rgba(249,115,22,0.48)] transition-shadow disabled:cursor-not-allowed disabled:opacity-70 border border-orange-400/30 cursor-pointer mt-2"
              >
